...
class UserProfile {
  ...
  UserProfile({
    ...
    required this.birthDate,import 'package:flutter/material.dart';
import 'package:dio/dio.dart';
...
class UserProfile {
  ...
  UserProfile({
    required this.firstName, required this.lastName, required this.image,
    required this.title, required this.email, required this.phone,
    required this.address, required this.birthDate, required this.gender,
    required this.bloodGroup, required this.height, required this.companyName,
    required this.department, required this.cardType, required this.cardNumber,
  });
...
class Bai11ProfileScreen extends StatelessWidget {
  ...
  Widget _buildSectionCard({required String title, required List<Widget> children}) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(title, style: const TextStyle(color: Color(0xFF9C27B0), fontSize: 18, fontWeight: FontWeight.bold)),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Hồ sơ người dùng", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
              ...
              children: [
                Center(
                  child: Column(
                    children: [
                      ...
                      Text("${user.firstName} ${user.lastName}", style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Color(0xFF4A148C))),
                      Text(user.title, style: const TextStyle(fontSize: 16, color: Colors.grey)),
                    ],
                  ),
                ),
                ...
                _buildSectionCard(
                  ...
                  children: [
                    ...
                    _buildRowInfo(Icons.height, "Chiều cao", "${user.height} cm"),
                  ],
                ),
                _buildSectionCard(
...
// ==========================================
// 3. MÀN HÌNH LOGIN (TÊN MỚI: Bai11LoginPage)
// ==========================================
...
class _Bai11LoginPageState extends State<Bai11LoginPage> {
  ...
  InputDecoration _inputDecoration({required String hintText, required IconData prefixIcon, Widget? suffixIcon}) {
    return InputDecoration(
      ...
      border: OutlineInputBorder(borderRadius: BorderRadius.circular(10.0), borderSide: const BorderSide(color: Colors.black54)),
      enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(10.0), borderSide: const BorderSide(color: Colors.black54)),
      focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(10.0), borderSide: const BorderSide(color: Color(0xFF9C27B0), width: 2)),
    );
  }
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          // Truyền ID sang màn hình profile
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFFF3E5F5),
      appBar: AppBar(
        title: const Text("Bài 11: Login & Profile", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
        centerTitle: true,
        backgroundColor: const Color(0xFF9C27B0),
        iconTheme: const IconThemeData(color: Colors.white),
      ),
          ...
          child: Column(
            ...
            children: [
              ...
              TextFormField(
                ...
                decoration: _inputDecoration(hintText: "Tài khoản (kminchelle)", prefixIcon: Icons.email_outlined),
              ...
              TextFormField(
                ...
                decoration: _inputDecoration(
                  ...
                  suffixIcon: IconButton(
                    icon: Icon(_isObscure ? Icons.visibility_off : Icons.visibility, color: Colors.grey[700]),
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: const Text("Đăng nhập", style: TextStyle(fontSize: 18, color: Colors.white)),
              ...
              const SizedBox(height: 20),
              const Text("TK: kminchelle | MK: 0lelplR", style: TextStyle(color: Colors.grey)),
            ],
          ),
        ),
      ),
...
// ==========================================
// 2. MÀN HÌNH PROFILE: Nhận userId từ màn hình trước
// ==========================================
class Bai11ProfileScreen extends StatelessWidget {
  final int userId; // <--- Biến bắt buộc này gây ra lỗi nếu không truyền vào
  const Bai11ProfileScreen({super.key, required this.userId});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder(
        // Sử dụng userId được truyền vào để gọi API
        future: Dio().get('https://dummyjson.com/users/$userId'),
        builder: (context, snapshot) {
          if (!snapshot.hasData)
            return const Center(
...
// ==========================================
// 3. MÀN HÌNH LOGIN (ĐÃ ĐỔI TÊN VÀ SỬA LỖI NAVIGATOR)
// ==========================================
class Bai11LoginPage extends StatefulWidget {
  const Bai11LoginPage({super.key});
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          int idFromServer = response.data['id']; // Lấy ID từ server

          Navigator.push(
            context,
            MaterialPageRoute(
              // Truyền userId vào màn hình Profile
              builder: (context) => Bai11ProfileScreen(userId: idFromServer),
            ),
          );
        }
